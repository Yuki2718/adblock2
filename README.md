...
Braveでの使用について：BraveはuBlock Origin文法と比較的高い互換性を持ちますが、現時点では`!#include`の未サポートによりサブリストが読み込まれません。これについては[uBO module](https://raw.githubusercontent.com/Yuki2718/adblock2/main/japanese/jpfp-ub.txt)のマニュアル追加で対応できますが、将来サポートしたときに冗長になります。つい最近、YoutubeでBraveの文法互換性による問題が起きたこともあり、公式にはサポートしない、自己責任でのご利用とさせていただきます。
